...
print(f'Load: {t1-t0:.2f}s  →  {portfolio.shape}')

from modules.module3_neglect import NeglectModule
t2 = time.time()
r = NeglectModule().run(portfolio)
t3 = time.time()
print(f'Module3: {t3-t2:.2f}s  →  neglected={len(r["data"])}, full={len(r["full_analysis"])}')
print(f'عدد العملاء: {"عدد العملاء" in r["full_analysis"].columns}')

from export.excel_writer_xl import ExcelReportWriter
out = r'c:\Users\dell\Downloads\فايلات مهاره\test_xlsxwriter_neglect.xlsx'
t4 = time.time()
...
writer.save()
t5 = time.time()
print(f'XlsxWriter write+save: {t5-t4:.2f}s')
print(f'TOTAL: {t5-t0:.2f}s')
sz = os.path.getsize(out) // 1024
print(f'File: {os.path.basename(out)}  ({sz} KB)')
